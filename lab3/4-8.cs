using System;
using System.IO;
using System.Collections.Generic;
using System.Xml.Serialization;
using System.Linq;

[Serializable]
public struct Toy{
    public string Name;
    public int Price;
    public int Minage;
    public int Maxage;
    public Toy(string name, int price, int minage, int maxage){
        Name = name;
        Price = price;
        Minage = minage;
        Maxage = maxage;
    }
}


class Program
{
    static Random R = new Random();

    public static void Main()
    {
        First();
        touchtoysfile();
        Console.WriteLine("Ответ на второе 5-e задание: " + maxtoy());
        zap();
        mission6();
        zapfor7();
        calc7();
        zapfor8();
    }
//задание 4
    public static void First()
    {
        string input = "input.bin";
        string output = "output.bin";
        FileStream f = new FileStream(input, FileMode.Create);
        BinaryWriter file = new BinaryWriter(f);
        int n = R.Next(5, 20);
        List<int> b = new List<int>();
        List<int> c = new List<int>();
        Console.WriteLine("Входной файл: ");
        for (int i = 0; i < n; i++)
        {
            int a = R.Next(0, 10);
            Console.Write(a + "\t");
            file.Write(a);
        }
        Console.WriteLine();
        
        file.Close();
        f.Close();
        f = new FileStream(input, FileMode.Open);
        BinaryReader readInput = new BinaryReader(f);
        while (readInput.BaseStream.Position < readInput.BaseStream.Length)
        {
            b.Add(readInput.ReadInt32());
        }

        readInput.Close();
        f.Close();

        foreach (int bb in b)
        {
            if (!c.Contains(bb))
            {
                c.Add(bb);
            }
        }

        f = new FileStream(output, FileMode.Create);
        file = new BinaryWriter(f);
        foreach (int i in c)
        {
            file.Write(i);
        }

        file.Close();
        f.Close();

        f = new FileStream(output, FileMode.Open);
        readInput = new BinaryReader(f);
        Console.WriteLine("Выходной файл: ");
        while (readInput.BaseStream.Position < readInput.BaseStream.Length)
        {
            int number = readInput.ReadInt32();
            Console.Write(number + "\t");
        }
        Console.WriteLine();
        readInput.Close();
        f.Close();
    }
    //задание 5
    static void touchtoysfile(){
        List<Toy> toys = new List<Toy>(){
            new Toy { Name = "Мяч", Price = 300, Minage = 1, Maxage = 5 },
            new Toy { Name = "Лего", Price = 1200, Minage = 1, Maxage = 10 },
            new Toy { Name = "Кукла", Price = 500, Minage = 2, Maxage = 5 },
            new Toy { Name = "Пазлы", Price = 800, Minage = 2, Maxage = 6 },
        };
        using(FileStream fs = new FileStream("toys.xml", FileMode.OpenOrCreate)){
            XmlSerializer xmlSerializer = new XmlSerializer(typeof(List<Toy>));
            xmlSerializer.Serialize(fs, toys);
        }
        toys = null;
    }
    static string maxtoy(){
        List<Toy> toys;
        string fss = "toys.xml";

        int min = 2;
        int max = 3;
        int maxprice = -9999;
        string result = null;
        using(FileStream ffss = new FileStream(fss, FileMode.Open)){
            XmlSerializer serializer = new XmlSerializer(typeof(List<Toy>));
            toys = (List<Toy>)serializer.Deserialize(ffss);
        }
        foreach(Toy i in toys){
            if(i.Minage <= min && i.Maxage >= max){
                if(i.Price > maxprice){
                    result = i.Name;
                    maxprice = i.Price;
                }
            }
        }
        if(result == null) return "Нет подходящего";
        else return result;
    }
    //заданиие 6
    static void zap(){
        string input2 = "input6.txt";
        FileStream f = new FileStream(input2, FileMode.Create);
        f.Close();
        StreamWriter ff = new StreamWriter(input2);
        int a = R.Next(10, 30);
        for(int i = 0; i < a; i++){
            int k = R.Next(10, 1000);
            string next = k.ToString() + "\n";
            ff.Write(next);
            next = "";
        }
        ff.Close();
    }
    static void mission6(){
        string input2 = "input6.txt";
        Console.WriteLine("Задание 6: Введите необходимое значение");
        int find = int.Parse(Console.ReadLine());
        int count = 0;
        StreamReader f = new StreamReader(input2);
        string line;
        while((line = f.ReadLine()) != null){
            if(line[line.Length-1]- '0'== find){
                count += Convert.ToInt32(line);
            }
        }
        Console.WriteLine("Заданиие 6: " + count);
    }
    //задание 7
    static void zapfor7(){
        string input7 = "input7.txt";
        FileStream f = new FileStream(input7, FileMode.Create);
        f.Close();
        StreamWriter fs = new StreamWriter(input7);
        int lines = R.Next(3, 10);
        int rows = R.Next(3,10);
        for(int i = 0; i < lines; i++){
            for(int j = 0; j < rows; j++){
                int rand = R.Next(1, 100);
                fs.Write(rand);
                if(j < rows -1){
                    fs.Write(" ");
                }
            }
            fs.WriteLine();
        }
        fs.Close();
    }
    static void calc7(){
        string input7 = "input7.txt";
        StreamReader fs = new StreamReader(input7);
        string firstline = fs.ReadLine();
        int min = 9999999;
        string[] firstlineel = firstline.Split(" ");
        int one = int.Parse(firstlineel[0]);
        while(!fs.EndOfStream){
            string line = fs.ReadLine();
            string[] num = line.Split(" ");
            foreach(string nubmer in num){
                int curr = int.Parse(nubmer);
                if(curr < min){
                    min = curr;
                }
            }
        }
        Console.WriteLine("Задание 7 \n Первое минус минимальное: " + (one -= min));
        fs.Close();
    }
    //Задание 8
    static void zapfor8()
    {
        string inputFilePath = "input8.txt";
        string outputFilePath = "output8.txt";
        char[] punctuation = { '.', ',', '!', '?', ';', ':', '-', '(', ')'};
        using (StreamReader reader = new StreamReader(inputFilePath))
        using (StreamWriter writer = new StreamWriter(outputFilePath))
        {
            string line;
            while ((line = reader.ReadLine()) != null)
            {
                if (line.IndexOfAny(punctuation) == -1)
                {
                    writer.WriteLine(line); 
                }
            }
        }
    }
}